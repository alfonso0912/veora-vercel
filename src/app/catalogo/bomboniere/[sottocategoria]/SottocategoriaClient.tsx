"use client";

import Link from "next/link";
import Image from "next/image";
import catalogoData from "@/app/catalogo/catalogoData";

type Prodotto = {
  nome: string;
  slug: string;
  immagini?: string[];
  prezzo?: string;
};

export default function SottocategoriaClient({
  sottocategoria,
}: {
  sottocategoria: string;
}) {
  // ✅ Normalizzazione per lookup (accenti/apostrofi)
  const normalizedForLookup = decodeURIComponent(sottocategoria)
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/['’]/g, "")
    .toLowerCase();

  // ✅ Chiave “pulita” per confronti robusti
  const cleanKey = normalizedForLookup.replace(/[^a-z0-9]/g, "");
  const isRicordiPreziosi = cleanKey === "ricordipreziosi";
  const isVasiDAutore = cleanKey === "vasidautore";

  const prodotti: Prodotto[] =
    (catalogoData["bomboniere"]?.sottocategorie as Record<string, Prodotto[]>)[
      normalizedForLookup
    ] || [];

  const titolo = decodeURIComponent(sottocategoria)
    .replace(/-/g, " ")
    .replace(/\b\w/g, (l) => l.toUpperCase())
    .replace(/\bD Autore\b/, "d'Autore");

  if (!prodotti.length) {
    return (
      <div className="min-h-screen flex items-center justify-center text-center bg-[#f8f6f1] px-6">
        <div>
          <h1 className="text-2xl font-bold text-[#c6a664] mb-3 uppercase">
            Sottocategoria non trovata
          </h1>
          <Link
            href="/catalogo/bomboniere"
            className="text-[#c6a664] font-semibold underline"
          >
            Torna alle Bomboniere
          </Link>
        </div>
      </div>
    );
  }

  return (
    <main className="min-h-screen bg-[#f8f6f1] pb-20 px-4 flex flex-col items-center">
      <h1
        className={`italic font-[Cormorant_Garamond] font-semibold
          bg-gradient-to-r from-[#b9935a] via-[#e5c97b] to-[#b9935a]
          bg-[length:300%_100%] text-transparent bg-clip-text
          animate-[fadeInUp_1.2s_ease-out_forwards,shimmer_8s_infinite_linear]
          tracking-wide leading-tight text-center mt-8 mb-10`}
        style={{
          fontSize: "clamp(1.6rem, 4vw, 2.6rem)",
          lineHeight: "1.1",
          whiteSpace: "normal",
          overflowWrap: "break-word",
          maxWidth: "92vw",
        }}
      >
        Collezione {titolo}
      </h1>

      <section className="w-full max-w-6xl grid grid-cols-2 md:grid-cols-4 gap-4">
        {prodotti.map((p, i) => (
          <Link
            key={i}
            href={`/catalogo/bomboniere/${sottocategoria}/${p.slug}`}
            className="bg-white rounded-2xl shadow-sm hover:shadow-md transition block overflow-hidden"
          >
            <div className="relative w-[92%] mx-auto aspect-square mt-2 rounded-xl overflow-hidden flex items-center justify-center bg-white">
              <Image
                src={p.immagini?.[0] || "/images/logo-veora.jpg"}
                alt={p.nome}
                fill
                className="object-contain"
                sizes="(max-width:768px)45vw,25vw"
                priority={i < 4}
              />
            </div>

            <div className="p-3 text-center">
              <h3
                className={`text-[15px] font-semibold leading-snug mb-1
                  bg-gradient-to-r from-[#b9935a] via-[#e5c97b] to-[#b9935a]
                  bg-clip-text text-transparent`}
              >
                {p.nome}
              </h3>

              {/* ✅ Ricordi Preziosi → box classico (come Festività) */}
              {isRicordiPreziosi && (
                <div
                  className="inline-block mt-1 mb-2 border border-[#c6a664] rounded-lg px-3 py-1 text-black font-semibold text-[14px] bg-[#fff8e7] shadow-sm"
                  style={{ minWidth: "fit-content" }}
                >
                  Prezzo {p.prezzo || "su richiesta"}
                </div>
              )}

              {/* ✅ Vasi d’Autore → box identico alla Home ma SENZA “Bomboniera”, CON “Prezzo” */}
              {isVasiDAutore && (
                <div
                  className="inline-block mt-1 mb-2 border border-[#c6a664] rounded-[10px] px-4 py-1.5 text-[#111] font-bold text-[15px] bg-[#f6d8b1] shadow-[0_2px_3px_rgba(0,0,0,0.08)]"
                  style={{ minWidth: "fit-content" }}
                >
                  Prezzo {p.prezzo || "su richiesta"}
                </div>
              )}
            </div>
          </Link>
        ))}
      </section>

      <style jsx global>{`
        @keyframes shimmer {
          0% {
            background-position: 0% center;
          }
          100% {
            background-position: 300% center;
          }
        }
        @keyframes fadeInUp {
          from {
            opacity: 0;
            transform: translateY(20px);
          }
          to {
            opacity: 1;
            transform: translateY(0);
          }
        }
      `}</style>
    </main>
  );
}
